    ...
    void emit(CodeWriter codeWriter, String enumName, Set<Modifier> implicitModifiers)
        ...
        try {
            ...
            for (Iterator<Map.Entry<String, TypeSpec>> i = enumConstants.entrySet().iterator();
                ...
                // Bootify - no extra line between values
            ...
            for (FieldSpec fieldSpec : fieldSpecs) {
                ...
                // Bootify - no extra line between fields
                // if (!firstMember) codeWriter.emit("\n");
            ...
            if (!staticBlock.isEmpty()) {
                ...
                firstMember = false;
                hasStatic = false; // Bootify no extra line after static block
            }

            // Non-static fields.
            final boolean simpleFields = fieldSpecs.stream().anyMatch(fs -> !fs.annotations.isEmpty());
            for (FieldSpec fieldSpec : fieldSpecs) {
                if (fieldSpec.hasModifier(Modifier.STATIC)) continue;
